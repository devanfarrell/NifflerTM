...
bool TM_Facade::getPosInt( int&result, std::string prompt)
{
  ...
  if(buffer >> ret_value && std::to_string(ret_value) == input && ret_value > 0) // check for garbage, truncation, and overflow
  {
    result = ret_value;
    validity = true;
  }
...
TM_Facade::TM_Facade(Validator * validator, TM_Definition * tmDefinition, Input_Strings * inputStrings, std::string fileName):
fileName(fileName)
{
...
void TM_Facade::help()
{
  std::cout << "D\t"<< "DELETE\t\t" << "Delete input string from list" << std::endl;
  std::cout << "X\t"<< "EXIT\t\t"  << "Exit application " << std::endl;
  std::cout << "H\t"<< "HELP\t\t"  << "Help user" << std::endl;
  std::cout << "I\t"<< "INSERT\t\t"  << "Insert input string into list" << std::endl;
  std::cout << "L\t"<< "LIST\t\t"  << "List input strings" << std::endl;
  std::cout << "Q\t"<< "QUIT\t\t"  << "Quit operation of Turing machine on input string" << std::endl;
  std::cout << "R\t"<< "RUN\t\t\t"  << "Run Turing machine on input string" << std::endl;
  std::cout << "E\t"<< "SET\t\t\t"  << "Set maximum number of transitions to perform" << std::endl;
  std::cout << "W\t"<< "SHOW\t\t"  << "Show status of application" << std::endl;
  std::cout << "T\t"<< "TRUNCATE\t"  << "Truncate instantaneous description" << std::endl;
  std::cout << "V\t"<< "VIEW\t\t"  << "View Turing machine" << std::endl;
}
void TM_Facade::insert()
{
  ...
  if(input.length() > 0)
  {
    ...
    else std::cout << "Error: input string contains characters not in sigma" << std::endl;
  }
}
void TM_Facade::list()
{
...
{
  int posInt = 0;
  std::string prompt = "Set maximum number of transitions [" + std::to_string(transitionProcessor->getMaxTransitions()) + "]: ";
...
{
  int posInt = 0;
  std::string prompt = "Set maximum number of cells [" + std::to_string(transitionProcessor->getMaxCells()) + "]: ";
  bool validInput = getPosInt(posInt, prompt);
  if(validInput)
  {
