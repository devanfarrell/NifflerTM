...
bool Tape::isFirstCell()
{
  ...
  if(currentCell==0) isFirstCell = true;
  return isFirstCell;
}
bool Tape::isLastCell()
{
  bool isLastCell = false;
  if(currentCell==cells.length() - 1) isLastCell = true;
  return isLastCell;
}
...
{
  unsigned long endCell = cells.length();
  endCell--;
  while((endCell >= currentCell) && (cells[endCell]==blankCharacter))
  {
    --endCell;
  }
  unsigned long lastCell = 0;
  if (endCell < currentCell + maxCellsNumber)
  {
    lastCell = endCell;
  }
  else lastCell = currentCell + maxCellsNumber;
