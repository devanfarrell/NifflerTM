...
// begin constructor
TM_Definition::TM_Definition(std::vector<std::string> description_in,
...
bool TM_Definition::searchTransition(std::string sourceState, char readCharacter,
                                     std::string& destinationState, char& writeCharacter, char& moveDirection) const
{
  bool found = false;
  for (size_t i = 0; i < transitionFunction.size(); i++)
  {
    if (sourceState == transitionFunction[i].getCurrentState()
        && readCharacter == transitionFunction[i].getReadCharacter())
    {
